...
def average_pooling(img, G=10):
    ...
    for y in range(Nh):
        for x in range(Nw):
             out[G*y:G*(y+1), G*x:G*(x+1)] = np.mean(out[G*y:G*(y+1), G*x:G*(x+1)]).astype(np.int)
    return out
...
try:
    while True:
        ...
        # AVG_Pooling depth image
        depth_pool_image = average_pooling(depth_image)

        # Apply colormap on depth image (image must be converted to 8-bit per pixel first)
        depth_colormap = cv2.applyColorMap(cv2.convertScaleAbs(depth_pool_image, alpha=0.03), cv2.COLORMAP_JET)
